...
@api_v1.route('/portfolio/export', methods=['POST'])
@api_access_logger
@require_rate_limit('/api/v1/portfolio/export')
@require_api_key('portfolio_export')
    ...
    try:
        ...
        for stock in stocks:
            try:
                ...
                # 分析股票
                analysis_result = analyzer.analyze_stock(normalized_code, market_type)
                ...
                # 基本面分析
                fundamental_result = fundamental_analyzer.analyze_fundamentals(normalized_code, market_type)
                ...
                logger.error(f"处理股票 {stock.get('stock_code', 'unknown')} 时出错: {str(e)}")
                continue
        ...
        logger.error(f"投资组合导出API出错: {traceback.format_exc()}")
        return APIResponse.error(
            code=ErrorCodes.INTERNAL_SERVER_ERROR,
            message='投资组合导出失败',
            details={'error_message': str(e)},
